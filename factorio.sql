...
-- 1, 2. Добавить в таблицу "Станция" столбец "Местоположение" типа geometry(Point). Сгенерировать случайное местоположение для всех станций.
alter table "Станция"
...
-- 3. Удалить значение из enum.
alter type "Тип станции"
rename to "Тип станции (удаляется)";

create type "Тип станции" as enum (
--  'Депо', /* теперь тоже Буферная стоянка */
  'АЗС',
  'Буферная стоянка',
  'Месторождение',
  'Предприятие',
...
update "Станция"
set "Тип" = 'Буферная стоянка'
where "Тип" = 'Депо';

alter table "Станция"
alter "Тип" type "Тип станции" using "Тип"::text::"Тип станции";

drop type "Тип станции (удаляется)";
...
-- 7. Найти маршруты с самым длинным временем простоя на станциях
with "Простаивающие" as (
  select
    "Маршрут"."Имя" as "Маршрут",
    "Станция"."Имя" as "Станция",
    avg(extract(epoch from ("Журнал"."Время отправления" - "Журнал"."Время прибытия"))/3600) as "Простаивает"
  from "Журнал"
  join "Логистика" on "Журнал"."Местоположение" = "Логистика"."Номер"
  join "Маршрут" on "Логистика"."Маршрут" = "Маршрут"."Номер"
  join "Станция" on "Логистика"."Станция" = "Станция"."Номер"
  where "Журнал"."Время отправления" is not null
  group by "Маршрут"."Имя", "Станция"."Имя"
)
select "Маршрут", "Станция", "Простаивает"
from "Простаивающие"
where "Простаивает" = (select max("Простаивает") from "Простаивающие");


-- 8. Найти маршруты, которые посещают станции с определенным типом груза (например, с углём)
select distinct "Маршрут"."Имя" as "Маршрут"
from "Логистика"
join "Маршрут" on "Логистика"."Маршрут" = "Маршрут"."Номер"
join "Станция" on "Логистика"."Станция" = "Станция"."Номер"
join "Хранилище" on "Станция"."Номер" = "Хранилище"."Станция"
join "Ячейка" on "Хранилище"."Ячейка" = "Ячейка"."Номер"
where "Ячейка"."Ресурс" = 'Уголь'
order by "Маршрут";


-- 9. Найти предприятия с самой низкой производительностью по каждому типу продукции
with "Статистика" as (
  select "Продукция", min("Производительность") as "Производительность"
  from "Предприятие"
  group by "Продукция"
)
select "Предприятие"."Тип", "Предприятие"."Продукция", "Предприятие"."Производительность"
from "Предприятие"
join "Статистика" on
  "Предприятие"."Продукция" = "Статистика"."Продукция" and
  "Предприятие"."Производительность" = "Статистика"."Производительность"
join "Станция" on "Предприятие"."Номер" = "Станция"."Номер";


-- 10. Найти станции с наибольшим средним временем простоя поездов
select "Станция"."Имя" as "Станция", "Станция"."Тип" as "Тип станции",
  round(avg(extract(epoch from ("Журнал"."Время отправления" - "Журнал"."Время прибытия"))/60)::numeric, 1) as "Простой",
  count(*) as "Кол-во остановок"
from "Журнал"
join "Логистика" on "Журнал"."Местоположение" = "Логистика"."Номер"
join "Станция" on "Логистика"."Станция" = "Станция"."Номер"
where "Журнал"."Время отправления" is not null
  and "Журнал"."Время отправления" > "Журнал"."Время прибытия"
group by "Станция"."Номер", "Станция"."Имя", "Станция"."Тип"
having count(*) > 3;
...
create or replace trigger "Проверка числа поездов"
before insert on "Журнал"
for each row
execute function "Проверить число поездов"();

-- p.s. попробуем заставить триггер ругаться
insert into "Журнал" ("Время прибытия", "Время отправления", "Локомотив", "Местоположение")
values (
  timestamp '2022-02-02 11:01:01',
  null,
...
-- 2. Проверка поездов и маршрутов перед удалением станции.
create or replace function "Удаление станции"()
returns trigger as $$
declare
  "Активные маршруты" integer;
  "Активные поезда" integer;
begin
  select count(*) into "Активные маршруты"
  from "Логистика"
  where "Станция" = old."Номер";

  select count(*) into "Активные поезда"
  from "Журнал"
  join "Логистика" on "Журнал"."Местоположение" = "Логистика"."Номер"
  where "Логистика"."Станция" = old."Номер"
  and "Журнал"."Время отправления" is null;

  if "Активные маршруты" > 0 or "Активные поезда" > 0 then
    raise exception 'Нельзя удалить станцию "%" (ID: %): % маршрутов и % поездов используют.',
      old."Имя", old."Номер", "Активные маршруты", "Активные поезда";
  end if;
...
create or replace trigger "Перед удалением станции"
before delete on "Станция"
for each row
execute function "Удаление станции"();


-- 3. Предотвратить обновление типа станции.
create or replace function "Предотвратить изменение типа станции"()
returns trigger as $$
begin
  if new."Тип" != old."Тип" then
    if old."Тип" in ('Месторождение', 'Предприятие') then
      raise exception 'Нельзя изменять тип станции для месторождений и предприятий';
    end if;

    if exists (
      select 1
      from "Журнал"
      join "Логистика" l on "Журнал"."Местоположение" = "Логистика"."Номер"
      where "Логистика"."Станция" = new."Номер"
      and "Журнал"."Время отправления" is null
    ) then
      raise exception 'Изменение типа невозможно, поезда не станции';
    end if;
  end if;
  return new;
end;
...
create or replace trigger "Изменение типа станции"
before update of "Тип" on "Станция"
for each row
execute function "Предотвратить изменение типа станции"();
